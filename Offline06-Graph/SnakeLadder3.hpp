...
public:
    ...
    void play(int start = 1)
    {
        ...
        playerPos = start;
        init(playerPos);
        q.push(make_pair(-dist[playerPos], playerPos));
        while (!q.empty())
        {
            playerPos = q.top().second;
            q.pop();
            if (adj[playerPos] != NIL)
            {
                if (isLadder(adj[playerPos]))
                {
                    moveUp(adj[playerPos]);
                }
                else if (isSnake(adj[playerPos]))
                {
                    slideDown(adj[playerPos]);
                }
            }
            roll();
        }
